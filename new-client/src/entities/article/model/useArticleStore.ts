...
interface UseArticleStore {
  ...
  loadPrevPage: () => void
  loadNextPage: () => void
  addArticle: (request: z.infer<typeof createArticleRequest>, setError: UseFormSetError<z.infer<typeof createArticleRequest>>) => Promise<boolean>
  getArticles: (pageNumber: number, pageSize: number) => Promise<void>
  getArticle: (articleId: string) => Promise<Article | undefined>
  updateArticle: (request: z.infer<typeof createArticleRequest>) => Promise<void>
...
export const useArticleStore = create<UseArticleStore>((set, get) => ({
  ...
  loadNextPage: () => {
    set(state => {
      if (state.pageNumber < Math.ceil(state.totalCount / state.pageSize)) {
        return { pageNumber: state.pageNumber + 1 };
      }
      return state
    })
  ...
  addArticle: async (request, setError) => {
    ...
    try {
      ...
      set(state => ({ articles: [response.data, ...state.articles] }))
      return true
    } catch (error) {
      console.error(error)
      return false
